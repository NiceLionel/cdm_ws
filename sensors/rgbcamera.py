...
def pygame_callback(image, side, renderObject):
    ...
    if (
        ...
        renderObject.surface = pygame.surfarray.make_surface(
            img_combined.swapaxes(0, 1)
        )
...
class cameraManage:
    def __init__(self, world, ego_vehicle, pygame_size):
        ...
        self.image_size_x = int(
            pygame_size.get("image_x") / 2
        )  # 横向放置两个摄像头的画面
        self.image_size_y = int(
            pygame_size.get("image_y") / 2
        )  # 纵向放置两个摄像头的画面
    ...
    def camaraGenarate(self):
        cameras_transform = [
            (
                carla.Transform(
                    carla.Location(x=2.0, y=0.0, z=1.3),  # 前侧摄像头安装位置
            ...
            (
                carla.Transform(
                    carla.Location(x=-2.0, y=0.0, z=1.3),  # 后侧摄像头安装位置
            ...
            (
                carla.Transform(
                    carla.Location(x=0.0, y=2.0, z=1.3),  # 左侧摄像头安装位置
            ...
            (
                carla.Transform(
                    carla.Location(x=0.0, y=-2.0, z=1.3),  # 右侧的摄像头安装位置
                    carla.Rotation(pitch=0, yaw=-90, roll=0),
                ),
                "Right",
            ),
        ...
        for index, (camera_ts, camera_sd) in enumerate(cameras_transform):
            camera = self.world.spawn_actor(
                camera_bp, camera_ts, attach_to=self.ego_vehicle
            )
...
if __name__ == "__main__":
    ...
    # 生成车辆并设置自动驾驶
    vehicle_bp = (
        world.get_blueprint_library().filter("*vehicle*").filter("vehicle.tesla.*")[0]
    )
